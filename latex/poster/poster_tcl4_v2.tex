...
\begin{center}
    \colorbox{nottblue!100}{%
        \begin{minipage}[t]{\textwidth} % Adjusted vertical alignment
		    ...
		    \Large \textit{\textcolor{white}{ \textbf{Prem Kumar} }}\textsuperscript{\textcolor{white}{1,2}}, \textit{\textcolor{white}{K. P. Athulya}}\textsuperscript{\textcolor{white}{1,2}}, \textit{\textcolor{white}{Sibasish Ghosh}}\textsuperscript{\textcolor{white}{1,2}}\\
    ...
    \vspace{0em} % Add vertical space
            ...
            \begin{center}
                \begin{tikzpicture}[remember picture,overlay]
                    ...
                    {\includegraphics[width=8cm,height=8cm]{img/hbnilogo.jpg}}; % Adjust position and image
                \end{tikzpicture}
            \end{center}
        \end{minipage}
    }
...
\vspace{-0.8cm}
% Abstract (own banner) + Background (own banner) in a four-column strip
\noindent
\begin{minipage}[t]{0.25\textwidth}
  \coloredsection{accentorange!70!white}{Abstract}{}
  \coloredsubsection{accentorange!10!white}{~}{
   We derive the full fourth-order TCL (TCL4) generator for the spin-boson model (SBM) for arbitrary odd physical spectral densities. We apply this result to solve for the dynamics of a semiconductor double quantum dot, which can be modeled as a SBM under certain conditions. Our results show that the commonly used TCL2 master equation often overestimates the non-Markovianity at large temperatures. We also benchmarking our results against analytical results for Ohmic spectral densities and the numerically exact Hierarchical Equations of Motion. These results offer a general and computationally efficient method for studying steady-state and dynamical properties of a wide class of open quantum systems.
  }
\end{minipage}
\hfill
\begin{minipage}[t]{0.72\textwidth}
  ...
  \begin{multicols}{3}
\begin{center}
	\posterheading{\textbf{TCL master equation.}}
\end{center}
  ...
  \end{align*}
  The zeroth-order part $\mathcal{F}^{(0)}(t)[\hat{\rho}(t)] = -i [\hat{H}_S, \hat{\rho}(t)]$ gives free evolution, $\mathcal{F}^{(2)}(t)$ is the Bloch–Redfield contribution, and higher even orders add systematic corrections. 
\columnbreak


\begin{center}
\posterheading{\textbf{Spin-boson model.}}
\end{center}
  The Hamiltonian is
  \begin{equation*}
...
\bgtitle{Bloch-vector TCL form.}\\
  In Bloch-vector notation,
  \begin{equation*}
  \dot{\mathbf{v}}(t) = \sum_{n=0}^{\infty} \lambda^{2n} \, F^{(2n)}(t) \, \mathbf{v}(t),
  \end{equation*}
  with matrix elements $$F^{(2n)}_{mn}(t) \equiv \operatorname{Tr}\!\{\hat{\sigma}_{m} \, F^{(2n)}(t)[ \hat{\sigma}_{n} ] \}$$. Here $\lambda$ sets the system–environment coupling; $\hat{\sigma}_i$ are Pauli matrices; $m_k,\omega_k,c_k$ are the mass, frequency, and coupling of the $k^{\text{th}}$ mode; and $a_{1},a_{3}$ encode detuning and tunneling.
...
\vspace{0.5cm}
\begin{itemize}
    \item[\circleicon{blue}] We quantify the non-Markovianity of TCL2 and TCL4 dynamics for the SBM using the BLP measure (Breuer, Laine \& Piilo, 2009) for an Ohmic spectral density with Drude cutoff $J_D(\omega) = \frac{\gamma  \Lambda ^2 \omega }{\Lambda ^2+\omega ^2}$.
...
\begin{figure}[H]
    \centering
    \begin{tabular}{cc}
        \includegraphics[width=0.48\linewidth]{img/AntiPtcl2lamdaTplot.pdf} &
        \includegraphics[width=0.48\linewidth]{img/AntiPtcl4lamdaTplot.pdf} \\
        \includegraphics[width=0.48\linewidth]{img/AntiPredifftcl.pdf} &
        \includegraphics[width=0.48\linewidth]{img/TCLNormPlot.pdf}
    \end{tabular}
    \caption{These figures plot the BLP measure ($N(\Phi)$) as logarithmic color plot calculated using (a) TCL2 and (b) TCL4 as a function of $\Lambda$ and $T$. Part (c) plots the difference between these two quantities ($N(\Phi_{\text{TCL4}})-N(\Phi_{\text{TCL2}})$), while (d) shows the $L_2$ norm ratio $||F^{(4)}||_2/||F^{(2)}||_2$, highlighting the regime where TCL4 remains valid. The black line marks the Markovian regime from the resonance condition. The BLP measure is maximized over $400$ antipodal Bloch sphere pairs.}%
\end{figure}
}
...
\begin{minipage}[t]{0.48\textwidth}
  ...
  \coloredsubsection{accentorange!10!white}{~}{
  \begin{itemize}
	\item[\circleicon{black}] We analytically derive TCL4 generator in large time limit for a generic SBM for arbitray odd physical spectral densities.
    \item[\circleicon{black}] For Ohmic-Drude spectral density, TCL2 overestimates non-Markovianity across a wide range of $T$ and $\Lambda$.
